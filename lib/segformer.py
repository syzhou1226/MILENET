    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if (len(self.reduction_ratio) == 4):
            ...
            if (N == 4704):
                ...
                tem2 = x[:, :980, :].reshape(B, 14, 14, C * 5).permute(0, 3, 1, 2)
                tem3 = x[:, 980:1372, :]
                sr_2 = self.sr2(tem2).reshape(B, C, -1).permute(0, 2, 1)
                reduce_out = self.norm(torch.cat([sr_2, tem3], -2))
...
class M_EfficientSelfAtten(nn.Module):
    def __init__(self, dim, head, reduction_ratio):
        super().__init__()
        self.head = head
        self.reduction_ratio = reduction_ratio  # list[2  4  1  8]
        self.scale = (dim // head) ** -0.5
        self.q = nn.Linear(dim, dim, bias=True)
        self.kv = nn.Linear(dim, dim * 2, bias=True)
        self.proj = nn.Linear(dim, dim)
...
class CascadeConv(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv3 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv1 = nn.Conv2d(dim, dim, 1, 1, 0, groups=dim)
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        tx1 = self.conv3(tx)
        tx2 = self.conv3(tx1)
        tx3 = self.conv1(tx) + tx2

        tx4 = self.conv3(tx3)
        tx5 = self.conv3(tx4)
        tx6 = self.conv1(tx3) + tx5

        return tx6.flatten(2).transpose(1, 2)
...
class LE_FFN(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fc1 = nn.Linear(c1, c2)
        self.cascadeConv = CascadeConv(c2)
        self.act = nn.GELU()
        self.fc2 = nn.Linear(c2, c1)
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ax = self.act(self.norm1(self.cascadeConv(self.fc1(x), H, W) + self.fc1(x)))
        out = self.fc2(ax)
        return out
