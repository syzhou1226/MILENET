...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    if len(image.shape) == 3:
        ...
        predictionA = np.zeros_like(label)
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                outA = torch.argmax(torch.softmax(outputsA, dim=1), dim=1).squeeze(0)
                outA = outA.cpu().detach().numpy()


                if x != patch_size[0] or y != patch_size[1]:


                    predA = zoom(outA, (x / patch_size[0], y / patch_size[1]), order=0)
        ...
        with torch.no_grad():
            ...
            predictionA = out.cpu().detach().numpy()
    metric_list = []
    for i in range(1, classes):

        metric_list.append(calculate_metric_percase(predictionA == i, label == i))

    if test_save_path is not None:
        img_itk = sitk.GetImageFromArray(image.astype(np.float32))
        prd_itk = sitk.GetImageFromArray(predictionA.astype(np.float32))
