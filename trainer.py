...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        """
        :param inputs: shape (N, C, H, W) - logits
        :param targets: shape (N, H, W) - int class labels
        """
...
def trainer_synapse(args, model, snapshot_path):
    ...
    ce_loss = FocalLoss(alpha=1.0, gamma=2.0, reduction='mean')
    ...
    for epoch_num in iterator:
        ...
        if(performance>=0.8):
            save_mode_path = os.path.join(snapshot_path, 'epoch={}_avg_dcs={}.pth'.format(epoch_num, performance))
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
